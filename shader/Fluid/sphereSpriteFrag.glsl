...
void main()
{
    ...
    {
        z = sqrt(z2);
        gl_FragDepth = gl_FragCoord.z ;//- (z/gl_FragCoord.z);
        ...
        if(densityRatio < 1.0)
        {
            densityColour  = mix(vec3(1,0,0), vec3(0.2,0.5,1), densityRatio);
        }
        else
        {
            densityColour  = mix(vec3(0.2,0.5,1), vec3(1,1,1), densityRatio-1.0);
        }
